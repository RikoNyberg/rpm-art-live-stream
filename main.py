...
def stream_dat_file(
    ...
    *,
    ...
    for batch_range in pacer.pace(src.ranges()):
        ...
        start_ts_us = int(ts_us[0]) if ts_us.size else int(getattr(batch_range, "start_ts_us", 0))
        end_ts_us = int(ts_us[-1]) if ts_us.size else int(getattr(batch_range, "end_ts_us", start_ts_us))
        yield EventWindow(
            x_coords=x_coords,
            y_coords=y_coords,
